...
use ps_sig::message_structure::vc::{Address, CredentialSubject, VC};
use ps_sig::rsssig::{RSVerifyResult, RSignature};
...
fn issuers_actions() -> (SignedVC, PKrss) {
    ...
    let encoded_msgs = vc.encode_demo();
...
fn holders_actions(signed_full_vc: SignedVC, issuer_pk: &PKrss) -> SignedVC {
    ...
    let math_idx_lookup = signed_full_vc.vc.field_idx_map_demo();
...
fn verifiers_actions(signed_redacted_vc: SignedVC, issuer_pk: &PKrss) -> RSVerifyResult {
    ...
    let encoded_msgs = signed_redacted_vc.vc.encode_demo();
    let math_idx_lookup = signed_redacted_vc.vc.field_idx_map_demo();
...
fn verify_full_vc(signed_full_vc: SignedVC, issuer_pk: &PKrss) -> RSVerifyResult {
    ...
    RSignature::verifyrsignature(
        issuer_pk,
        &full_sig,
        signed_full_vc.vc.encode_demo().as_slice(),
        &idxs,
    )
}
