...
pub struct RSSKeyPair {
    pub public_key: PKrss,
    pub private_key: SKrss,
}
...
impl SKrss {
    ...
    pub fn from_hex(sk: &str) -> Result<SKrss, SKrssError> {
        SKrss::from_bytes(&hex::decode(sk)?)
    }
}
...
impl From<hex::FromHexError> for PKrssError {
    fn from(value: hex::FromHexError) -> Self {
        PKrssError::FailedParsingHex(value)
    }
}
...
impl PKrss {
    ...
    pub fn from_hex(pk: &str) -> Result<PKrss, PKrssError> {
        PKrss::from_bytes(&hex::decode(pk)?)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_rsskey_hex_conversion() {
        let (sk, pk) = rsskeygen(3, &Params::new("test".as_bytes()));
        assert_eq!(sk, SKrss::from_hex(&sk.to_hex()).unwrap());
        assert_eq!(pk, PKrss::from_hex(&pk.to_hex()).unwrap());
    }
